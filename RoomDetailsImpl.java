...
public class RoomDetailsImpl implements IRoomDetailsService {
	...
	public RoomDetails updateRoomDetails(RoomDetails roomDetails,Integer room_id) {
		...
		if( existingRoomDetails.isPresent())
		{
			...
			roomDetails1.setAvailable(true);
	...
	@Override
	public RoomDetails removeRoomDetails(Integer room_id) 
	{
	...
	public RoomDetails showRoomDetails(Integer room_id) {
		return room_Detailsrepository.findById(room_id).orElseThrow(
				() -> new RoomDetailsNotFoundException("RoomDetails with Id " + room_id +" not found"));
	}
}
